...
public class RSAHelper {
	...
	public void initKey(int keysize) throws Exception {
		try {
		  InputStream prips = new FileInputStream(privatePath+"pkcs8_rsa_private_key_2048.pem");
      InputStream puips = new FileInputStream(publicPath+"xlzx_rsa_public_key_2048.pem");
			localPrivKey = getPrivateKey(prips);
			peerPubKey = getPublicKey(puips);
	...
	public RSAPublicKey getPublicKey(String publicKeyStr) throws Exception {
		try {
			byte[] buffer = Base64.decodeBase64(publicKeyStr);//Base64Utils.decodeFromString(publicKeyStr);
	...
	public RSAPrivateKey getPrivateKey(String privateKeyStr) throws Exception {
		try {
		  //new sun.misc.BASE64Decoder().decodeBuffer(privateKeyStr);
